...
Node = class {
	...
	**/
	static makeNodeFromXdslEl(el, $xdsl, opts) {
		opts = opts || {};
	...
	*/
	...
	duplicateLocal(o = {}) {
		...
		/// Make child
		var child = new Node({
			id: this.id, def: this.def.duplicate(),
			states, statesById,
			parents: newParents
		});
	...
	addStates(newStates, opts) {
		...
		this.def.updateStates({
			oldNumStates: numStates, newNumStates: numNewStates,
	...
	removeStates(statesToRemove, opts) {
		opts = opts || {};
		var updateChildren = typeof (opts.updateChildren) == "undefined" ? true : opts.updateChildren;
		var oldChildren = [];
	...
	**/
	...
	_addParent(parent, { noUpdate } = { noUpdate: false }) {
	...
	_addChild(child, { noUpdate } = { noUpdate: false }) {
		var prevChild = child;
	...
	setValues(values, { updateStateNames } = { updateStateNames: true }) {
		funcTable = [];
		for (var i in values) {
	...
	*/
	...
	duplicateInto(node, extraProps) {
		...
		var propsToDuplicate = {
			label: 1, parents: 1, children: 1, def: 1, values: 1,
	...
	*/
	...
	@includeThis: (boolean) Whether to also check if this node is in |nodes|.
	*/
	hasDescendent(nodes, includeThis) {
		if (typeof (nodes) != "object" || !('length' in nodes)) nodes = [nodes];
		for (var i = 0; i < nodes.length; i++) {
			if (typeof (nodes[i]) == "string") nodes[i] = this.net.nodesById[nodes[i]];
		}
...
TextBox = class {
	...
	setSubmodelPath(...args) {
		return Node.prototype.setSubmodelPath.apply(this, args);
	}
	getSubmodelPathStr(...args) {
		return Node.prototype.getSubmodelPathStr(this, args);
	}
	moveToSubmodel(path) {
		var node = this;
		path = BN.makeSubmodelPath(path);
	...
	renameToUnique(...args) {
		Node.prototype.renameToUnique.apply(this, args);
	}
}

/// Almost all the methods are the same as TextBox. Need to refactor
ImageBox = class extends TextBox {
	static { globalThis.ImageBox = this; }
	static DisplayItem = addMixin(this, DisplayItem);
	constructor(o = {}) {
		this._type = "ImageBox";
...
Submodel = class {
	static { globalThis.Submodel = this; }
	static DisplayItem = addMixin(this, DisplayItem);
	static Convert = addMixin(this, Convert);
	static convert = {
		toJSON: {
	...
	*/
	...
	setSubmodelPath(...args) { return Node.prototype.setSubmodelPath.apply(this, args); }
	getSubmodelPathStr(...args) { return Node.prototype.getSubmodelPathStr.apply(this, args); }
	getSubmodelParent(...args) { return Node.prototype.getSubmodelParent.apply(this, args); }
	/// Move this node to the given submodel. |path| is a left-to-right array
...
BN = class extends Submodel {
	static __newBnIndex = 1;
	static { globalThis.BN = this; }
	static Convert = addMixin(this, Convert);
	static convert = {
		toJSON: {
	...
	static defaultIterations = mbConfig.iterations;
	static updateMethodTitles = {
	...
	constructor(o = {}) {
		...
		console.log("BNINDEX", BN.__newBnIndex);
		...
		o.format = o.format ?? (o.fileName ? BN.getFileType(o.fileName) : null) ?? "xdsl";
		...
		/// Will be false if an updateBeliefs is in progress
		this.ready = true;
	...
	*/
	...
	static makeSubmodelPathStr(pathArr) {
		return pathArr ? '/' + pathArr.concat(['']).join('/') : '';
	}
	static sortItems(items, sortType) {
	...
	init(o = {}) {
		...
		this.fileName = this.fileName || "bn" + (BN.__newBnIndex++) + ".xdsl";
...
`;
	...
	async load_cmp(cmpText) {
		...
		for (let jNode of jNodes) {
			...
			for (let i = 0; i < oldCpt.length; i++) {
				...
				let newI = leftGroupI + rightGroupOffset + state;
	...
	*/
	...
	learnStructureNaiveBayes(data, classNode) {
		var bn = this;
	...
	*/
	...
	learnStructureTan(data, classNode) {
		var bn = this;
	...
	calculateMiStats(data, classVar) {
		/// Initialisation
		var marginalCounts = {};
		var pairs = {};
		var pairsByClass = {};
	...
	updateBeliefs_likelihoodWeightingWorker(callback, iterations) {
		(async () => {
			try {
				console.log('worker');
	...
	calcProbabilityOfEvidence(callback) {
	...
	**/
	...
	findAllPathsBetweenNodes(sourceNode, destNode) {
	...
	areNodesDConnected(sourceNode, destNode) {
	...
	isConnectedPath(path, checkFrom) {
		//return true;
	...
	findAllDConnectedNodes2(sourceNode, o = {}) {
	...
	findAllDConnectedNodes(sourceNode) {
		/// To Do
		var bn = this;
		if (typeof (sourceNode) == "string") sourceNode = this.nodesById[sourceNode];
		var connectedNodes = {};
